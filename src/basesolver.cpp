...
FresnelCoeffs::FresnelCoeffs(const CMatrix& R_perp, const CMatrix& R_para) :
  perp{R_perp},
  para{R_para}
{}
...
GFCoeff::GFCoeff(const CMatrix& c1,
  const CMatrix& c2,
  const CMatrix& c3,
  const CMatrix& c4,
  const CMatrix& c5,
...
void BaseSolver::calculateGFCoeffRatios(const FresnelCoeffs& fresnelCoeffs,
  CMatrix& CB,
  CMatrix& FB,
  CMatrix& CT,
  CMatrix& FT)
{
  ...
  for (Eigen::Index i = matstack.numLayers - mDipoleLayer - 2; i >= 0; --i) {
    ...
    CT.row(i) =
      ...
      (fresnelCoeffs.perp.row(indexFromTop) + (CT.row(i + 1) * Eigen::exp(-2.0 * I * matstack.h.row(indexFromTop + 1) *
                                                                          (matstack.z0.cast<CMPLX>())(indexFromTop)))) /
      (1 + fresnelCoeffs.perp.row(indexFromTop) *
             (CT.row(i + 1) *
               Eigen::exp(-2.0 * I * matstack.h.row(indexFromTop + 1) * (matstack.z0.cast<CMPLX>())(indexFromTop))));

    FT.row(i) =
      Eigen::exp(2.0 * I * matstack.h.row(indexFromTop) * (matstack.z0.cast<CMPLX>())(indexFromTop)) *
      (-fresnelCoeffs.para.row(indexFromTop) +
        (FT.row(i + 1) *
          Eigen::exp(-2.0 * I * matstack.h.row(indexFromTop + 1) * (matstack.z0.cast<CMPLX>())(indexFromTop)))) /
      (1 - fresnelCoeffs.para.row(indexFromTop) *
             (FT.row(i + 1) *
               Eigen::exp(-2.0 * I * matstack.h.row(indexFromTop + 1) * (matstack.z0.cast<CMPLX>())(indexFromTop))));
  }
}

void BaseSolver::calculateGFCoeffs(const GFCoeffRatios& gfCoeffRatios,
  CMatrix& c,
  CMatrix& cd,
  CMatrix& f_perp,
  CMatrix& fd_perp,
  ...
  CMatrix& fd_para)
{
  ...
  CMPLX I(0.0, 1.0);
  c.row(mDipoleLayer) = (gfCoeffRatios.cb.row(mDipoleLayer) + 1) * gfCoeffRatios.ct.row(0) /
                        (1 - gfCoeffRatios.cb.row(mDipoleLayer) * gfCoeffRatios.ct.row(0));
  cd.row(mDipoleLayer) = (gfCoeffRatios.ct.row(0) + 1) * gfCoeffRatios.cb.row(mDipoleLayer) /
                         (1 - gfCoeffRatios.cb.row(mDipoleLayer) * gfCoeffRatios.ct.row(0));

  f_perp.row(mDipoleLayer) = (gfCoeffRatios.fb.row(mDipoleLayer) + 1) * gfCoeffRatios.ft.row(0) /
                             (1 - gfCoeffRatios.fb.row(mDipoleLayer) * gfCoeffRatios.ft.row(0));
  fd_perp.row(mDipoleLayer) = (gfCoeffRatios.ft.row(0) + 1) * gfCoeffRatios.fb.row(mDipoleLayer) /
                              (1 - gfCoeffRatios.fb.row(mDipoleLayer) * gfCoeffRatios.ft.row(0));

  f_para.row(mDipoleLayer) = (gfCoeffRatios.fb.row(mDipoleLayer) - 1) * gfCoeffRatios.ft.row(0) /
                             (1 - gfCoeffRatios.fb.row(mDipoleLayer) * gfCoeffRatios.ft.row(0));
  fd_para.row(mDipoleLayer) = (1 - gfCoeffRatios.ft.row(0)) * gfCoeffRatios.fb.row(mDipoleLayer) /
                              (1 - gfCoeffRatios.fb.row(mDipoleLayer) * gfCoeffRatios.ft.row(0));
...
void BaseSolver::calculateLifetime(const GFCoeff& gfCoeff, Vector& bPerp, Vector& bPara)
{
  ...
  bTmp *= (gfCoeff.f_perp(mDipoleLayer, Eigen::seqN(0, matstack.u.size() - 1)) +
           gfCoeff.fd_perp(mDipoleLayer, Eigen::seqN(0, matstack.u.size() - 1)));
  bPerp = bTmp.real();

  bTmp2 = (gfCoeff.c(mDipoleLayer, Eigen::seqN(0, matstack.u.size() - 1)) +
           gfCoeff.cd(mDipoleLayer, Eigen::seqN(0, matstack.u.size() - 1)));
  bTmp = Eigen::pow(Eigen::sin(matstack.x.head(matstack.x.size() - 1)), 2);
  bTmp *= (gfCoeff.f_para(mDipoleLayer, Eigen::seqN(0, matstack.u.size() - 1)) -
           gfCoeff.fd_para(mDipoleLayer, Eigen::seqN(0, matstack.u.size() - 1)));
  bTmp += bTmp2;
...
void BaseSolver::calculateDissPower(const GFCoeff& gfCoeff, const double bPerpSum)
{
  ...
  for (Eigen::Index i = 0; i < matstack.numLayers - 1; ++i) {
    mPowerPerpU.row(i) = (-3.0 * q * matstack.dU / 4.0) *
    ...
    mPowerPerpU.row(i) *= (gfCoeff.f_perp(i, Eigen::seqN(0, mPowerPerpU.cols())) *
                            Eigen::exp(-I * matstack.h.row(i) * (matstack.z0.cast<CMPLX>())(i))) -
                          ((gfCoeff.fd_perp(i, Eigen::seqN(0, mPowerPerpU.cols())) + boolValue(i)) *
                            Eigen::exp(I * matstack.h.row(i) * (matstack.z0.cast<CMPLX>())(i)));
    mPowerPerpU.row(i) *= (Eigen::conj((gfCoeff.f_perp(i, Eigen::seqN(0, mPowerPerpU.cols())) *
                                         Eigen::exp(-I * matstack.h.row(i) * (matstack.z0.cast<CMPLX>())(i))) +
                                       ((gfCoeff.fd_perp(i, Eigen::seqN(0, mPowerPerpU.cols())) + boolValue(i)) *
                                         Eigen::exp(I * matstack.h.row(i) * (matstack.z0.cast<CMPLX>())(i)))));

    mPowerParaU.row(i) = (-3.0 * q * matstack.dU / 8.0) *
                         (matstack.u.segment(0, matstack.u.size() - 1) *
                           Eigen::conj(Eigen::sqrt(matstack.epsilon(i) / matstack.epsilon(mDipoleLayer) -
                                                   Eigen::pow(matstack.u.segment(0, matstack.u.size() - 1), 2)))) /
                         (Eigen::abs(1 - Eigen::pow(matstack.u.segment(0, matstack.u.size() - 1), 2)));
    mPowerParaU.row(i) *= (gfCoeff.c(i, Eigen::seqN(0, mPowerParaU.cols())) *
                            Eigen::exp(-I * matstack.h.row(i) * (matstack.z0.cast<CMPLX>())(i))) +
                          ((gfCoeff.cd(i, Eigen::seqN(0, mPowerParaU.cols())) + boolValue(i)) *
                            Eigen::exp(I * matstack.h.row(i) * (matstack.z0.cast<CMPLX>())(i)));
    mPowerParaU.row(i) *= (Eigen::conj((gfCoeff.c(i, Eigen::seqN(0, mPowerParaU.cols())) *
                                         Eigen::exp(-I * matstack.h.row(i) * (matstack.z0.cast<CMPLX>())(i))) -
                                       ((gfCoeff.cd(i, Eigen::seqN(0, mPowerParaU.cols())) + boolValue(i)) *
                                         Eigen::exp(I * matstack.h.row(i) * (matstack.z0.cast<CMPLX>())(i)))));

    powerParaTemp.row(i) = (-3.0 * q * matstack.dU / 8.0) *
                           (matstack.u.segment(0, matstack.u.size() - 1) *
                             Eigen::sqrt(matstack.epsilon(i) / matstack.epsilon(mDipoleLayer) -
                                         Eigen::pow(matstack.u.segment(0, matstack.u.size() - 1), 2))) *
                           (std::conj(std::sqrt(matstack.epsilon(i))) / std::sqrt(matstack.epsilon(i)));
    powerParaTemp.row(i) *= (gfCoeff.f_para(i, Eigen::seqN(0, mPowerParaU.cols())) *
                              Eigen::exp(-I * matstack.h.row(i) * (matstack.z0.cast<CMPLX>())(i))) -
                            ((gfCoeff.fd_para(i, Eigen::seqN(0, mPowerParaU.cols())) - boolValue(i)) *
                              Eigen::exp(I * matstack.h.row(i) * (matstack.z0.cast<CMPLX>())(i)));
    powerParaTemp.row(i) *= (Eigen::conj((gfCoeff.f_para(i, Eigen::seqN(0, mPowerParaU.cols())) *
                                           Eigen::exp(-I * matstack.h.row(i) * (matstack.z0.cast<CMPLX>())(i))) +
                                         ((gfCoeff.fd_para(i, Eigen::seqN(0, mPowerParaU.cols())) - boolValue(i)) *
                                           Eigen::exp(I * matstack.h.row(i) * (matstack.z0.cast<CMPLX>())(i)))));
  }
